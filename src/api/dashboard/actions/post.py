...
async def _relate_chart_to_dashboard(
    chart_id: UUID, dashboard_id: UUID, account_id: UUID, db: AsyncSession
):
    await _get_chart_container(id=chart_id, account_id=account_id, db=db)
    ...
    async with db as session:
        ...
        result = await obj_dal.create(dashboard_id=dashboard_id, chart_id=chart_id)
        return result
